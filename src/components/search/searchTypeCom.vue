<template>
  ...
  <view class="search-result-box">
    ...
    <u-cell-group title="你可能感兴趣" class="cell-group-box" :border="false">
      ...
      >
        ...
        <view slot="title">
          ...
          <view style="font-size: 26rpx; color: #999"
            >粉丝：{{ formatNumber(item.fansSize) }} 歌曲：{{
              item.musicSize
            }}</view
          >
        </view>
      ...
      >
        ...
        <view slot="title">
          ...
          >
          <view style="font-size: 26rpx; color: #999">
            {{ item.artistName }},{{ item.duration / 1000 / 60 }},播放：{{
    ...
    <u-cell-group
      ...
      v-show="
        searchResultCom.song &&
        searchResultCom.song.songs &&
        searchResultCom.song.songs.length > 0
      "
    >
      <view
        style="
      ...
      >
        <view slot="title">
          ...
          <view style="font-size: 26rpx; color: #999"
            >{{ item.ar ? item.ar[0].name : "未知" }} - {{ item.al.name }}</view
          >
        </view>
      ...
      <u-cell-item
        :arrow="false"
        :border-bottom="false"
        v-if="searchResultCom.song.moreText"
      >
    ...
    <u-cell-group
      ...
      class="cell-group-box"
      v-if="
        searchResultCom.playList &&
        searchResultCom.playList.playLists &&
        searchResultCom.playList.playLists.length > 0
      "
    >
      <view
        style="
      ...
      <u-cell-item
        ...
        :arrow="false"
        v-for="item in searchResultCom.playList.playLists"
      ...
      >
        ...
        <view slot="title">
          <view style="font-size: 32rpx; color: #000000" class="only-line-1">{{
            item.name
          }}</view>
          <view style="font-size: 26rpx; color: #999">
      ...
      <u-cell-item
        :arrow="false"
        :border-bottom="false"
        v-if="searchResultCom.playList.moreText"
      >
        <view style="text-align: center">
    ...
    <u-cell-group
      ...
      class="cell-group-box"
      v-if="
        searchResultCom.video &&
        searchResultCom.video.videos &&
        searchResultCom.video.videos.length > 0
      "
    >
      <view
        style="
      ...
      <u-cell-item
        ...
        :arrow="false"
        v-for="item in searchResultCom.video.videos"
        :key="item.coverUrl"
      >
        <view slot="icon" style="margin-right: 20rpx">
          <u-image
        ...
        <view slot="title">
          ...
          <view style="font-size: 26rpx; color: #999">
            {{ toFixed(item.durationms / 60 / 1000) }}，by
            {{ item.creator ? item.creator[0].userName : "未知" }},
            {{ formatNumber(item.playTime) }}播放
          </view>
        </view>
      </u-cell-item>
      ...
      <u-cell-item
        :arrow="false"
        :border-bottom="false"
        v-if="searchResultCom.video.moreText"
      >
        <view style="text-align: center">
