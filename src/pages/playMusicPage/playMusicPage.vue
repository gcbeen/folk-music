    ...
    <view class="music-content">
      <view class="center-box" @tap.stop="onClickCenterBox">
        ...
        >
          ...
          >
            ...
            <view
              class="lrc-wrapper"
              v-show="audioLrc.ms && audioLrc.ms.length > 0"
            >
              <view
      ...
      <view class="bot-buttons">
        ...
        <view class="process-bar-wrapper">
          <!-- 已播放 -->
          <view
            >{{ parseMinute(audioMeta.currentTime) }}:{{
              parseSecond(audioMeta.currentTime)
            }}</view
          >
          ...
          <view class="process-line-box">
            ...
            <!-- 缓存进度 -->
            <view
              class="pro-line buffed-line"
              :style="{ width: audioBuffedPerc + '%' }"
            />

            <!-- 已播放长度 -->
            <view
              class="pro-line play-line"
              :style="{ width: audioPlayPerc + '%' }"
            />
          </view>

          <!-- 总长度 -->
          <view
            >{{ parseMinute(audioMeta.duration) }}:{{
              parseSecond(audioMeta.duration)
            }}</view
          >
        </view>
        ...
        <view class="control-bts">
          <view style="margin: 0 20rpx"
            ><u-icon name="rewind-left" size="100"
          /></view>
          <view @click="onClickPlayBt" style="margin: 0 20rpx">
            <u-icon
              :name="audioMeta.paused ? 'play-circle' : 'pause-circle'"
              size="100"
            />
          </view>
          <view style="margin: 0 20rpx"
            ><u-icon name="rewind-right" size="100"
          /></view>
        </view>
      </view>
    </view>
